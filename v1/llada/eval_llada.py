...
class LLaDAEvalHarness(LM):
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            frechet_margin: Safety margin delta for Frechet-profile decoding (new).
        '''
        super().__init__()
        ...
        if LLaDAModelLM is None and (use_cache or dual_cache):
            raise RuntimeError(
                "LLaDA cache modes require a local cache-capable "
                "`model.modeling_llada.LLaDAModelLM` implementation. This checkout "
                "does not include it, so loading `GSAI-ML/LLaDA-8B-Instruct` through "
                "Hugging Face remote code will fail with: `The kvcache is not "
                "suppotred for MDM.` Run without `use_cache=True`, or restore the "
                "local `v1/llada/model/` package from the Fast-dLLM/LLaDA cache "
                "implementation before running prefix/dual cache experiments."
            )
        model_cls = LLaDAModelLM if LLaDAModelLM is not None else AutoModel
        self.model = model_cls.from_pretrained(model_path, trust_remote_code=True, torch_dtype=torch.bfloat16, config=config, **model_kwargs)
        self.model.eval()
        ...
        self.factor = factor
        self.frechet_margin = frechet_margin
    ...
    def generate_until(self, requests):
            ...
            if self.use_cache:
                if self.dual_cache:
                    generated_answer, nfe = generate_with_dual_cache(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length,
                                            ...
                                            frechet_margin=self.frechet_margin)
                else:
        ...
        if self.show_speed:
            ...
            if num_nfe > 0:
                print(f"Avg tokens per NFE: {num_tokens / num_nfe}")

...
class DreamGenerationMixin:
    ...
    def diffusion_generate(
        ...
        **kwargs,
        ...
        frechet_margin = kwargs.get("frechet_margin", None)
    ...
    def _sample(
        ...
        frechet_margin: Optional[float] = None,
    ) -> Union[DreamModelOutput, torch.LongTensor]:
        # init values
        ...
        for num_block in range(num_blocks):
            ...
            while True:
                ...
                if frechet_margin is not None:
                    # Fréchet-profile decoding for Dream
                    ...
                    # Fréchet-profile scoring
                    B_fc, L_fc = full_confidence.shape
                    values_fc, idx_fc = torch.sort(full_confidence, dim=1, descending=True)
                    counts_fc = (full_confidence > -torch.inf).sum(dim=1)
                    cols_fc = torch.arange(L_fc, device=full_confidence.device).unsqueeze(0).expand(B_fc, L_fc)
                    valid_fc = cols_fc < counts_fc.unsqueeze(1)
                    
                    sorted_conf_fc = torch.where(valid_fc, values_fc, torch.zeros_like(values_fc))
                    cum_fc = torch.cumsum(sorted_conf_fc, dim=1)
                    frechet_lb = cum_fc - cols_fc.to(cum_fc.dtype)
                    competitor_ub = 1.0 - values_fc
                    score_fc = frechet_lb - competitor_ub
                    score_fc = torch.where(valid_fc, score_fc, torch.full_like(score_fc, -torch.inf))
                    score_fc[:, 0] = torch.inf  # always transfer at least one
                    
                    choose_fc = score_fc > frechet_margin
                    topn_fc = choose_fc.sum(dim=1).clamp(min=1)
                    select_sorted_fc = cols_fc < topn_fc.unsqueeze(1)
                    
                    transfer_int_fc = torch.zeros(B_fc, L_fc, device=full_confidence.device, dtype=torch.int8)
                    transfer_int_fc = transfer_int_fc.scatter(1, idx_fc, select_sorted_fc.to(torch.int8))
                    transfer_index_fc = transfer_int_fc.bool()
                    
                    if dual_cache:
                        x[:, current_block_start:current_block_end][transfer_index_fc] = x_[transfer_index_fc]
                    else:
                        x[:, current_block_start:][transfer_index_fc] = x_[transfer_index_fc]

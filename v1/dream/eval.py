...
class Dream(LM):
    ...
    def _generate_batch(self, prompts: List[str]) -> List[str]:
        ...
        gen_tokens = (generation_ids.sequences[0][prompt_ids.shape[1]:] != self.tokenizer.eos_token_id).sum().item()
        if self.show_speed and gen_elapsed > 0:
            self.total_gen_time = getattr(self, 'total_gen_time', 0) + gen_elapsed
            self.total_gen_tokens = getattr(self, 'total_gen_tokens', 0) + gen_tokens
            self.total_nfe = getattr(self, 'total_nfe', 0) + getattr(self.model, '_last_nfe', 0)

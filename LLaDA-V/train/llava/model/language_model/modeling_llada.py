...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    @staticmethod
    def select_transfer_index(confidence, mask_index, num_transfer_tokens=None, threshold=None, factor=None, frechet_margin=None):
        '''
        Select masked positions to transfer for fixed-step, threshold, factor, or
        Fréchet-profile parallel decoding.
        '''
        ...
        if threshold is not None:
            ...
            return (transfer_index | force_mask) & mask_index
        ...
        if factor is not None:
            ...
            score = sorted_confidence - (1.0 - (factor / (n + 1.0)))
    ...
    def generate(self, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
                ...
                threshold=None, factor=None, frechet_margin=None):
        '''
        Args:
        ...
        for num_block in range(num_blocks):
            ...
            i = 0
            while True:
                if threshold is None and factor is None and frechet_margin is None and i >= steps:
                    break
                mask_index = (x == mask_id)
                if not mask_index[:, prompt.shape[1] + num_block * block_length: prompt.shape[1] + (num_block + 1) * block_length].any():
                    break
                if cfg_scale > 0.:
                    un_x = x.clone()
                ...
                transfer_index = self.select_transfer_index(
                    confidence,
                    mask_index,
                    num_transfer_tokens[:, min(i, steps - 1)] if (threshold is None and factor is None and frechet_margin is None) else None,
                    threshold=threshold,
                    factor=factor,
    ...
    def generate_with_embeds(self, inputs_embeds, steps=128, gen_length=128, block_length=128, temperature=0.,
        ...
        threshold=None, factor=None, frechet_margin=None, **kwargs):
        '''
        Args:

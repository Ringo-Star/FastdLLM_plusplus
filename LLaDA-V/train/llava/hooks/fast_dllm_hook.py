...
class FastDLLMGenerationHook:
    ...
    def _fast_generate_with_embeds(
        ...
        **kwargs
        ...
        """
        cfg_scale = kwargs.pop("cfg", cfg_scale)
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            for num_block in range(num_blocks):
                ...
                adaptive_decode = threshold is not None or factor is not None or frechet_margin is not None

                while True:
                    if not adaptive_decode and i >= steps:
                        break
                    ...
                    if i % prefix_refresh_interval == 0:
                        x0, transfer_index = self._get_transfer_index(
                            ...
                            num_transfer_tokens[:, min(i, steps - 1)] if not adaptive_decode else None,
                            found_stop_seq, stop_position, block_end, suffix_len, threshold, factor, frechet_margin
                        )
                        ...
                        x0, transfer_index = self._get_transfer_index(
                            ...
                            num_transfer_tokens[:, min(i, steps - 1)] if not adaptive_decode else None,
                            found_stop_seq, stop_position - block_start, block_end - block_start, suffix_len, threshold, factor, frechet_margin
                        )
                        x0_embeds = self.model.model.embed_tokens(x0)
    ...
    def _get_transfer_index(self, logits, temperature, remasking, mask_index, x, num_transfer_tokens, 
                          found_stop_seq, stop_position, block_end, suffix_len, threshold=None, factor=None, frechet_margin=None):
        ...
        transfer_index = self._select_transfer_index(
            ...
            threshold=threshold,
            factor=factor,
            frechet_margin=frechet_margin,
        )

        return x0, transfer_index

    @staticmethod
    def _select_transfer_index(confidence, mask_index, num_transfer_tokens=None, threshold=None, factor=None, frechet_margin=None):
        transfer_index = torch.zeros_like(mask_index, dtype=torch.bool, device=mask_index.device)
        masked_count = mask_index.sum(dim=1)
        if not (masked_count > 0).any():
        ...
        # Backward-compatible Fast-dLLM behavior: existing scripts pass
        # threshold=1 to mean the factor-style criterion from Fast-dLLM.
        dynamic_factor = threshold if threshold is not None else factor

        if dynamic_factor is not None:
            valid = cols < masked_count.unsqueeze(1)
            sorted_confidence = torch.where(valid, values, torch.zeros_like(values))
            n = cols.to(sorted_confidence.dtype) + 1.0
            score = sorted_confidence - (1.0 - (dynamic_factor / (n + 1.0)))
            score = torch.where(valid, score, torch.full_like(score, -torch.inf))
            score[:, 0] = torch.inf
            topn = (score >= 0).sum(dim=1).clamp(min=1)
        elif frechet_margin is not None:
            valid = cols < masked_count.unsqueeze(1)
            sorted_confidence = torch.where(valid, values, torch.zeros_like(values))
            cum = torch.cumsum(sorted_confidence, dim=1)
            frechet_lb = cum - cols.to(cum.dtype)
            competitor_ub = 1.0 - values
            score = frechet_lb - competitor_ub
            score = torch.where(valid, score, torch.full_like(score, -torch.inf))
            score[:, 0] = torch.inf
            topn = (score > frechet_margin).sum(dim=1).clamp(min=1)
        else:
            if num_transfer_tokens is None:
                raise ValueError("num_transfer_tokens must be set when no adaptive decode mode is used.")
            if num_transfer_tokens.dim() == 2 and num_transfer_tokens.size(1) == 1:
                num_transfer_tokens = num_transfer_tokens.squeeze(1)
            topn = num_transfer_tokens.to(device=confidence.device, dtype=torch.long).clamp(min=0)

        topn = torch.minimum(topn, masked_count)
        select_sorted = cols < topn.unsqueeze(1)
        transfer_int = torch.zeros(B, L, device=confidence.device, dtype=torch.int8)
        transfer_int = transfer_int.scatter(1, idx, select_sorted.to(torch.int8))
        return transfer_int.bool() & mask_index

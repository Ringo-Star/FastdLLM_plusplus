        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            if self.tokenizer is None:
                if self.version == "llama_v3":
                    tokenizer_path = os.getenv("LLAMA3_TOKENIZER_PATH", "meta-llama/Meta-Llama-3-8B-Instruct")
                    self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, trust_remote_code=True, local_files_only=_offline_local_files_only())
                elif self.version == "llava_llada":
                    tokenizer_path = os.getenv("LLADA_V_TOKENIZER_PATH", "GSAI-ML/LLaDA-V")
                    self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, trust_remote_code=True, local_files_only=_offline_local_files_only())
                else:
